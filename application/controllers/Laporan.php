...
class Laporan extends CI_Controller {
	...
	public function export_aset($id_lokasi,$tahun_perolehan)
	{
		...
		foreach($aset as $item) {
			...
			->setCellValue('D' . $kolom, $item->nama_satuan)
	...
	public function printPenghapusan($id_lokasi,$tahun_perolehan)
	{
		...
		$data['title'] = 'Laporan Penghapusan Aset';
	...
	public function export_penghapusan($id_lokasi,$tahun_perolehan)
	{
		...
		foreach($aset as $item) {
			...
			->setCellValue('A' . $kolom, $nomor)
			->setCellValue('B' . $kolom, $item->nama_barang)
			->setCellValue('C' . $kolom, $item->jumlah)
			->setCellValue('D' . $kolom, $item->nama_satuan)
			->setCellValue('E' . $kolom, $item->harga)
			->setCellValue('F' . $kolom, $item->total_harga);
	...
	public function printPengadaan($id_lokasi,$tahun_pengadaan)
	{
		...
		$this->load->view('report/header',$data);
		$this->load->view('laporan/p_pengadaan');
		$this->load->view('report/footer');
	}
	...
	public function mutasi_aset()
	{
		$data = array(
			...
			'active_menu_lp' => 'menu-open',
			'active_menu_lpr' => 'active',
			'active_menu_lm' => 'active',
			'item' => $this->mt->getDataMutasi()  
		);
		$this->load->view('layouts/header',$data);
		$this->load->view('laporan/mutasi/index',$data);
		$this->load->view('layouts/footer');
	}

	public function print_mutasi_aset()
	{
		$data = array(
			'title' => 'Laporan Mutasi Aset',
			'item' => $this->mt->getDataMutasi()  
		...
		$this->load->view('report/header',$data);
		$this->load->view('laporan/mutasi/print');
		$this->load->view('report/footer');
	}

	public function export_mutasi_aset()
	{
		$aset = $this->mt->getDataMutasi();
		...
		$spreadsheet->setActiveSheetIndex(0)
					...
					->setCellValue('E1', 'KE USER')
					->setCellValue('F1', 'KE LOKASI')
		...
		foreach($aset as $item) {
			...
			->setCellValue('A' . $kolom, $nomor)
			->setCellValue('B' . $kolom, $item['nama_barang'])
			->setCellValue('C' . $kolom, $item['nama_user_m'])
			->setCellValue('D' . $kolom, $item['lokasi_m'])
			->setCellValue('E' . $kolom, $item['nama_user'])
			->setCellValue('F' . $kolom, $item['nama_lokasi'])
			->setCellValue('G' . $kolom, $item['tgl_mutasi'])
			->setCellValue('H' . $kolom, $item['status']);
		...
		header('Content-Type: application/vnd.ms-excel');
		header('Content-Disposition: attachment;filename="Data Mutasi Aset.xlsx"');
		header('Cache-Control: max-age=0');
	...
	public function peminjaman_aset()
	{
		$data = array(
			...
			'active_menu_lp' => 'menu-open',
			'active_menu_lpr' => 'active',
			'active_menu_lpj' => 'active',
			'item' => $this->mp->getDataPeminjaman() 
		);
		$this->load->view('layouts/header',$data);
		$this->load->view('laporan/peminjaman/index',$data);
		$this->load->view('layouts/footer');
	}

	public function print_peminjaman_aset()
	{
		$data = array(
			'title' => 'Laporan Peminjaman Aset',
			'item' => $this->mp->getDataPeminjaman()   
		...
		$this->load->view('report/header',$data);
		$this->load->view('laporan/peminjaman/print');
		$this->load->view('report/footer');
	}

	public function export_peminjaman_aset()
	{
		$aset = $this->mp->getDataPeminjaman();
		...
		foreach($aset as $item) {
			...
			->setCellValue('A' . $kolom, $nomor)
			->setCellValue('B' . $kolom, $item['nama_barang'])
			->setCellValue('C' . $kolom, $item['jml_pinjam'])
			->setCellValue('D' . $kolom, $item['kondisi_pinjam'])
			->setCellValue('E' . $kolom, $item['no_pinjam'])
			->setCellValue('F' . $kolom, $item['tgl_pinjam'])
			->setCellValue('G' . $kolom, $item['tgl_kembali'])
			->setCellValue('H' . $kolom, $item['status']);
		...
		header('Content-Type: application/vnd.ms-excel');
		header('Content-Disposition: attachment;filename="Data Peminjaman Aset.xlsx"');
		header('Cache-Control: max-age=0');
